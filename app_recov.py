...
if "last_product_shown" not in st.session_state:
    st.session_state.last_product_shown = None
...
with col2:
    if st.button(
        "💬 Parler à un agent commercial", disabled=st.session_state.agent_mode
    ):
...
if st.session_state.photo_mode:
    ...
    #camera_image = st.camera_input("Ou prendre une photo")

    if image_file :#or camera_image:
        image = image_file #if image_file else camera_image
        st.image(image, caption="Image reçue", use_container_width=True)
        st.session_state.messages.append(
...
if st.session_state.agent_mode:
    if st.button("❌ Annuler l'agent"):
        st.session_state.agent_mode = False

    message_contact = request_contact()
    agent_message = (
        "💬 Un agent commercial va prendre le relais.\n\n"
        "📞 Contactez-nous sur WhatsApp ou laissez un message.\n\n"
        f"{message_contact}"
    )
   

    # On ajoute les messages dans st.session_state.messages, pas d'affichage direct ici
    if not any(msg.get("content") == agent_message for msg in st.session_state.messages):
        st.session_state.messages.append({"role": "assistant", "content": agent_message})
        st.session_state.messages.append({"role": "assistant", "content": message_contact})


    st.markdown('<div class="chat-container">', unsafe_allow_html=True)

# ======================================================
# HISTORIQUE DU CHAT + AFFICHAGE AGENT
# ======================================================
                ...
                if isinstance(response, dict):
                    ...
                    if response["type"] == "product_image":
                        ...
                        if product.get("in_stock"):
                            ...
                            st.warning("❌Cet articcle est en rupture de stock")
                    ...
                    elif response["type"] == "request_contact":
                        contact_msg = request_contact()
                        st.info(contact_msg)
                        st.session_state.messages.append(
                            {"role": "assistant", "content": contact_msg}
                        )
